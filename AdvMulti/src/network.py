...
class Network(object):
    def __init__(self, config, summarizer):
        ...
        self.Wz1, self.Wz2, self.bz1, self.bz2 = self.init_Fz_variables()
    ...
    def init_Fe_variables(self):
        W1 = self.weight_variable([self.config.labels_dim + self.config.solver.randomvar_dim, self.config.solver.hidden_dim], "weight_e1")
    ...
    def init_Fz_variables(self):
        W1 = self.weight_variable([self.config.solver.randomvar_dim, self.config.solver.hidden_dim], "weight_z1")
        W2 = self.weight_variable([self.config.solver.hidden_dim, self.config.solver.latent_embedding_dim], "weight_z2")
        b1 = self.bias_variable([self.config.solver.hidden_dim], "bias_z1")
        b2 = self.bias_variable([self.config.solver.latent_embedding_dim], "bias_z2") 
        return W1, W2, b1, b2
    ...
    def Fd(self, input, keep_prob):
     ...
     def Fz(self, Z, keep_prob):
        hidden1 = tf.nn.dropout(utils.leaky_relu(tf.matmul(Z, self.Wz1)) + self.bz1, keep_prob)
        pred = tf.nn.dropout(utils.leaky_relu(tf.matmul(hidden1, self.Wz2) + self.bz2), keep_prob)
        return pred
    ...
    def discriminator(self, fake, real):
        disc_loss = tf.reduce_mean(-tf.log(real) - tf.log(1.0 - fake))
    ...
    def loss(self, features, labels, random_var, keep_prob):
        prediction, Fx, Fe, Fz = self.prediction(features, keep_prob), self.Fx(features, keep_prob), self.Fe(labels, keep_prob), self.Fz(Z, keep_prob)
        DiscFx, DiscFe, DiscFz = self.discriminator_network(Fx), self.discriminator_network(Fe), self.discriminator_network(Fz)
        disc_x_loss, gen_x_loss, disc_y_loss, gen_y_loss = self.discriminator(DiscFx, DiscFz), self.discriminator(DiscFe, DiscFz)
        disc_loss = disc_x_loss + gen_x_loss + disc_y_loss + gen_y_loss
        ...
        loss =  disc_loss + gen_loss + self.output_loss(tf.nn.sigmoid(prediction), labels)
        return loss

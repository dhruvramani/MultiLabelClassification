...
class Model(object):
    def __init__(self, config):
        ...
        self.loss = self.net.loss(self.x, self.y, self.z, self.keep_prob)
    ...
    def add_placeholders(self):
        ...
        self.z = tf.placeholder(tf.float32, shape=[None, self.config.solver.randomvar_dim])
    ...
    def run_epoch(self, sess, data, summarizer, epoch):
        ...
        for X, Y, tot in self.data.next_batch(data):
            feed_dict = {self.x : X, self.y : Y, self.z : np.random.random((Y.shape[0], self.config.solver.randomvar_dim)), self.keep_prob : self.config.solver.dropout}

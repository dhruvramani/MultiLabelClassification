...
class Network(object):
    ...
    def Fx(self, X, keep_prob):
        h1 = tf.nn.dropout(self.leakyRelu(tf.matmul(X, self.Wx1) + self.bx1), keep_prob)
        h2 = tf.nn.dropout(self.leakyRelu(tf.matmul(h1, self.Wx2) + self.bx2), keep_prob)
        return tf.nn.dropout(self.leakyRelu(tf.matmul(h2, self.Wx3) + self.bx3), keep_prob)

    def Fe(self, Y, keep_prob):
        h1 = tf.nn.dropout(self.leakyRelu(tf.matmul(Y, self.We1 + self.be1)), keep_prob)
        return tf.nn.dropout(self.leakyRelu(tf.matmul(h1, self.We2) + self.be2), keep_prob)

    def Fd(self, X, keep_prob):        
        labels_embedding = self.Fx(X, keep_prob)
        h1 = tf.nn.dropout(self.leakyRelu(tf.matmul(labels_embedding, self.Wd1) + self.bd1), keep_prob)
        return tf.nn.dropout(tf.matmul(h1, self.Wd2) + self.bd2, keep_prob)

    def predict(self, X, keep_prob):
        return self.Fd(X, keep_prob)

    def foo(self, X, Y):
        Fx, Fe = self.Fx(X, keep_prob), self.Fe(Y, keep_prob)
        return tf.reduce_mean(tf.square(Fx - Fe))

    def label_embedding_loss(self, X, Y, lagrange_const, keep_prob):
        '''
        Fx, Fe = self.Fx(X, keep_prob), self.Fe(Y, keep_prob)
        idenX = tf.constant(np.identity(self.config.batch_size), dtype=tf.float32), \
    ...
    def output_loss(self, X, Y, keep_prob):
        '''
        Ei = 0.0
        prediction = self.predict(X, keep_prob)
    ...
    def loss(self, X, Y, lagrange_const, alpha, keep_prob):
        '''
        total_loss = self.label_embedding_loss(X, Y, lagrange_const, keep_prob) + alpha * self.output_loss(X, Y, keep_prob) 
        return total_loss + #/ self.config.batch_size
        '''
        predictions = self.predict(X, keep_prob)
        labels = Y
